...
class GenerateTraj(Node):
 ...
 └── Publie: /planned_path (nav_msgs/Path)
    ...
    def publish_path(self, path_map):
        ...
        self.wp_pub = self.create_publisher(PoseArray, 'planned_path', 10)
        self.wp_pub.publish(msg)

        self.get_logger().info(
            f"{len(msg.poses)} poses publiées dans PoseArray"
        )
...
def main(args=None):
    ...
    map_path = os.path.expanduser("~/robmob_ws/src/params/map_inflated.pgm")
